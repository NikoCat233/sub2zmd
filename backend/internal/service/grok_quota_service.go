...
// ProbeUsage keeps its historical name for API compatibility. It performs the
// same read-only billing requests as grok-shell and does not consume quota.
func (s *GrokQuotaService) ProbeUsage(ctx context.Context, accountID int64) (*GrokQuotaProbeResult, error) {
	...
	if grokAccountUserID(account) == "" {
		...
		if account.Credentials == nil { account.Credentials = make(map[string]any) }
...
func (s *GrokQuotaService) fetchUserID(ctx context.Context, account *Account, token, proxyURL string) (string, error) {
	...
	if err != nil { return "", infraerrors.Newf(http.StatusInternalServerError, "GROK_USER_REQUEST_BUILD_FAILED", "failed to build user request: %v", err) }
	setGrokCLICommonHeaders(req, token)
	resp, err := s.httpUpstream.Do(req, proxyURL, account.ID, maxInt(account.Concurrency, 1))
	if err != nil { return "", infraerrors.Newf(http.StatusBadGateway, "GROK_USER_REQUEST_FAILED", "user request failed: %v", err) }
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode >= 400 { return "", infraerrors.Newf(mapUpstreamStatus(resp.StatusCode), "GROK_USER_UPSTREAM_ERROR", "user request returned %d", resp.StatusCode) }
	var payload struct { UserID string `json:"userId"` }
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&payload); err != nil { return "", infraerrors.Newf(http.StatusBadGateway, "GROK_USER_RESPONSE_INVALID", "invalid user response: %v", err) }
	payload.UserID = strings.TrimSpace(payload.UserID)
	if payload.UserID == "" { return "", infraerrors.New(http.StatusBadGateway, "GROK_USER_ID_MISSING", "Grok /user response did not include userId") }
	return payload.UserID, nil
}

func (s *GrokQuotaService) fetchBilling(ctx context.Context, account *Account, token, proxyURL string, credits bool) (*GrokBillingConfig, int, error) {
	target := xai.DefaultCLIBaseURL + "/billing"
	if credits { target += "?format=credits" }
	callCtx, cancel := context.WithTimeout(ctx, grokQuotaUpstreamTimeout)
	defer cancel()
	req, err := http.NewRequestWithContext(callCtx, http.MethodGet, target, nil)
	if err != nil { return nil, 0, infraerrors.Newf(http.StatusInternalServerError, "GROK_QUOTA_REQUEST_BUILD_FAILED", "failed to build billing request: %v", err) }
	setGrokCLIHeaders(req, account, token)
	resp, err := s.httpUpstream.Do(req, proxyURL, account.ID, maxInt(account.Concurrency, 1))
	if err != nil { return nil, 0, infraerrors.Newf(http.StatusBadGateway, "GROK_QUOTA_REQUEST_FAILED", "billing request failed: %v", err) }
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode >= 400 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 240))
		return nil, resp.StatusCode, infraerrors.Newf(mapUpstreamStatus(resp.StatusCode), "GROK_QUOTA_UPSTREAM_ERROR", "billing returned %d: %s", resp.StatusCode, truncate(strings.TrimSpace(string(body)), 240))
	}
	var payload grokBillingResponse
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&payload); err != nil {
		return nil, resp.StatusCode, infraerrors.Newf(http.StatusBadGateway, "GROK_QUOTA_RESPONSE_INVALID", "invalid billing response: %v", err)
	}
...
func grokAccountUserID(account *Account) string {
	...
	for _, key := range []string{"user_id", "userId", "sub"} {
		if userID := credentialString(account.Credentials, key); userID != "" { return userID }
	}
	if userID, _ := account.Extra["grok_user_id"].(string); strings.TrimSpace(userID) != "" { return strings.TrimSpace(userID) }
	return ""
}
...
func grokBillingSnapshot(config *GrokBillingConfig, status int) *xai.QuotaSnapshot {
	...
	if config.CurrentPeriod != nil {
		if end, err := time.Parse(time.RFC3339, config.CurrentPeriod.End); err == nil {
			unix, formatted := end.Unix(), end.UTC().Format(time.RFC3339)
			window.ResetUnix, window.ResetAt = &unix, formatted
		}
	}
	snapshot.Requests = window
	snapshot.HeadersObserved = true // Legacy field means usable upstream quota was observed.
...
func (s *GrokQuotaService) ResetQuota(ctx context.Context, accountID int64) (*GrokQuotaResetResult, error) {
	if _, err := s.loadGrokOAuthAccount(ctx, accountID); err != nil { return nil, err }
...
func (s *GrokQuotaService) prepareProbe(ctx context.Context, accountID int64) (*Account, string, string, error) {
	if s == nil || s.tokenProvider == nil || s.httpUpstream == nil { return nil, "", "", infraerrors.New(http.StatusInternalServerError, "GROK_QUOTA_NOT_CONFIGURED", "grok quota service is not configured") }
	account, err := s.loadGrokOAuthAccount(ctx, accountID); if err != nil { return nil, "", "", err }
	token, err := s.tokenProvider.GetAccessToken(ctx, account)
	if err != nil || strings.TrimSpace(token) == "" { return nil, "", "", infraerrors.Newf(http.StatusBadGateway, "GROK_QUOTA_TOKEN_UNAVAILABLE", "failed to acquire access token: %v", err) }
...
func (s *GrokQuotaService) resolveProxyURL(ctx context.Context, account *Account) string {
	...
	if account.Proxy != nil { return account.Proxy.URL() }
	if s != nil && s.proxyRepo != nil { if proxy, err := s.proxyRepo.GetByID(ctx, *account.ProxyID); err == nil && proxy != nil { return proxy.URL() } }
	return ""
}

func (s *GrokQuotaService) loadGrokOAuthAccount(ctx context.Context, accountID int64) (*Account, error) {
	if s == nil || s.accountRepo == nil { return nil, infraerrors.New(http.StatusInternalServerError, "GROK_QUOTA_NOT_CONFIGURED", "grok quota service is not configured") }
	account, err := s.accountRepo.GetByID(ctx, accountID)
	if err != nil || account == nil { return nil, infraerrors.Newf(http.StatusNotFound, "GROK_QUOTA_ACCOUNT_NOT_FOUND", "account not found: %v", err) }
	if account.Platform != PlatformGrok { return nil, infraerrors.New(http.StatusBadRequest, "GROK_QUOTA_INVALID_PLATFORM", "account is not a Grok account") }
	if account.Type != AccountTypeOAuth { return nil, infraerrors.New(http.StatusBadRequest, "GROK_QUOTA_INVALID_TYPE", "account is not an OAuth account") }
	return account, nil
}
