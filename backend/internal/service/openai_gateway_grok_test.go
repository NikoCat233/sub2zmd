...
func TestBuildGrokResponsesRequestUsesAccountBaseURLAndBearerToken(t *testing.T) {
	...
	require.Equal(t, grokCLITokenAuth, req.Header.Get("X-XAI-Token-Auth"))
	require.Equal(t, grokCLIClientVersion, req.Header.Get("x-grok-client-version"))
	require.Equal(t, grokCLIUserAgent, req.Header.Get("User-Agent"))
